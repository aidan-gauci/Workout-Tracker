...
window.addEventListener('DOMContentLoaded', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (gymDaysDropdown && workoutLogContainer) {
        ...
        workoutLogContainer.addEventListener('click', (event) => {
            ...
            if (deleteBtn) {
                const row = deleteBtn.closest('.workout-log-entry');
                const exerciseId = parseInt(row.getAttribute('data-exercise-id') || '0', 10);
                if (document.querySelectorAll('.workout-log-entry').length > 1) {
                    ...
                    if (deleteBtn.dataset.warning === 'true')
                        return;
            ...
            if (delSetBtn) {
                const dropdownContainer = delSetBtn.closest('.dropdown-content');
                ...
                if (setEntries.length > 1) {
                    ...
                    if (delSetBtn.dataset.warning === 'true')
                        return;
            ...
            if (addSetBtn) {
                const dropdownContainer = addSetBtn.closest('.dropdown-content');
                const setEntries = dropdownContainer.querySelectorAll('.set-entry');
                const row = dropdownContainer.closest('.workout-log-entry');
                const exerciseId = parseInt(row.getAttribute('data-exercise-id') || '0', 10);
                if (setEntries.length < 5) {
                    const newSetNum = setEntries.length + 1;
                    addSetBtn.parentElement.insertAdjacentHTML('beforebegin', createNewSet(newSetNum));
                    const exState = activeWorkoutState.find((ex) => ex.exercise_id === exerciseId);
                    if (exState)
                        exState.sets.push({ num: newSetNum, reps: 0, weight: 0 });
                }
                else {
                    if (addSetBtn.dataset.warning === 'true')
                        return;
